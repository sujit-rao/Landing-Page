...
function Navbar() {
    ...
    return (
        <>
                ...
                <div className=' text-xl container mx-auto flex justify-between items-center font-medium'>
                    ...
                    <div className=' md:hidden'>
                        <button onClick={toggleMenu} className=' text-white focus:outline-none focus:text-gray-300 '>
                            {
                                isMenuOpen ? (<FaXmark className=' h-6 w-6 text-primary' />) : (<FaBars className=' h-6 text-primary' />)
                            }
            ...
            {
                                navItems.map(({ link, path }) => <Link activeClass='active' spy={true} smooth={true} offset={-90} key={link} to={path} className=' block hover:text-gray-300 text-white' onClick={toggleMenu}>{link}</Link>)
                            }
